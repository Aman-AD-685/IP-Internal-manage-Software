...
def build_signup_confirmation_email(
    *,
    ...
    """Returns (subject, html_content, plain_fallback). Same shell as password reset."""
    email_safe = _esc(recipient_email)
    url_safe = _esc(confirm_url)
    url_plain = (confirm_url or "").strip()
    parsed = urlparse(url_plain)
    host_hint = parsed.netloc or "our secure sign-in page"

    subject = f"{PRODUCT_NAME} — confirm your email"

    plain = (
        f"{PRODUCT_NAME} — confirm your email\n\n"
        f"We received a sign-up for {recipient_email.strip()}.\n"
        f"If you did not create this account, ignore this email.\n\n"
        f"Confirm your email (expires in {expires_hours} hours):\n"
        f"{url_plain}\n\n"
        f"This link opens {host_hint} and then returns you to {PRODUCT_NAME}.\n\n"
        f"— {PRODUCT_NAME}\n"
        "Automated message — do not reply."
    ...
    inner = f"""
      ...
      <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#334155;">
        Use the button below to confirm your email. The link expires in
