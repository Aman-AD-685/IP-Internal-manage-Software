...
- 2026-09-07: **Production Sign Up** — keep login Sign Up visible; `VITE_ALLOW_PUBLIC_REGISTER=1` in `.env.production`; backend `ALLOW_PUBLIC_REGISTER` on unless explicitly `0`. Do not commit local `backend/Dockerfile` port 8020. Signup confirmation mail: Postmark + `generate_link` type `signup` *before* the user exists (not `create_user` then generate_link — that fails already-registered and skips Postmark). Confirm URL is SPA `/confirmation-success?token=` (Gmail prefetch). Already-registered unconfirmed users must Resend.
- 2026-09-08: **Signup rollback** — `_do_register` must delete the `user_profiles` row + auth user and return 503 when the Postmark confirmation mail fails; otherwise `generate_link` type `signup` returns "already registered" forever and the account is unreachable. `delete_auth_user` checks `email_confirmed_at` via `get_user_by_id` first — never roll back a confirmed account. Real reason lands in Render logs as `REGISTER ROLLBACK <uid>: ...`.
- 2026-09-08: **Signup orphan paths (all three)** — `signup_create_user_and_confirm_url` and `_signup_via_create_user` must `delete_auth_user` before every `raise` that happens after the user exists, not just on mail-send failure; otherwise register returns 503 while the account sits in `auth.users` and retries hit "already registered". Self-check uses a fake supabase admin and asserts the delete actually fires (source-grep asserts do not catch this).
